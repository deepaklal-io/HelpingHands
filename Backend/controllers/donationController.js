...
export const createDonation = async (req, res) => {
  try {
    ...
    // ✅ Prevent student from donating to their own request
    ...
    if (request.status !== "approved") {
      return res.status(400).json({
        message: "Only approved requests can receive donations",
      });
    }
    ...
    if (amount > remainingAmount) {
      return res.status(400).json({
        message: `Maximum donation allowed is PKR ${remainingAmount}`,
      });
    }
